...
1. **Build a five.** Choose five players by position (PG, SG, SF, PF, C) from your roster, plus bench depth. This is what finally makes the game a real 5-on-5: five distinct players on the floor, with synergies between them. The lineup is the only routine pre-game decision: there is no manual game plan to set. The one exception is the SHOWCASE call, a single toggle offered only when a chase legend is in the five on a qualifying game (see [signature-signings.md](signature-signings.md)); it bends the auto-derived plan toward that legend's signature moment at an honest cost.
