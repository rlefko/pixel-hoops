...
- **Rarity-gated reward bursts**: a reward pickup (boost draft pick, gear drop) fires a screen shake, color flash, and haptic scaled by rarity, so a capstone or a hot jackpot roll lands louder than a common, and a class promotion pops the player's tier badge. Built from the shared `src/feel` hooks (`useRewardBurst`) and gated by `FeelSettings`, so the juice scales with the reward instead of being a flat beat on everything. The draft's reroll and banish controls each fire their own small burst through the same hook, so the new agency lands as feedback rather than a silent button.
