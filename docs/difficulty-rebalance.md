...
- A drafted legend is **fielded scaled to the ladder**, not at unscaled all-time-great power. `legendDraftTargetLevel` (`src/game/classes.ts`) targets **two classes above the ladder** and `scaleLegendsForLadder` (`src/game/apply-effects.ts`) brings the line down to it, never up. This is the player-side mirror of the scaled boss legend above: a legend fields as a genuine star (e.g. A-level on the C ladder) and only reaches full power once you climb to the **S / S+ ladders**, where the target meets or exceeds its natural OVR and the scale is a no-op. So the collection's true OVR still shows everywhere, but the fielded (and draft-displayed) strength is ladder-appropriate. A flat 2-point unscaled legend used to near-clear a low ladder outright (outside the tuned bands); scaling makes it a reward for the climb instead of a shortcut through it.
- At most **one reach-up legend** per rotation (`MAX_DRAFT_LEGENDS`): a single franchise cornerstone, never a super-team dragged onto a low ladder. Only legends *above* the ladder count, so the S+ ladder, whose native class is legends, is uncapped and can still field a full five.
