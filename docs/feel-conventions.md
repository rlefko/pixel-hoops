...
2. The watch stays short by default; only named, bounded spends add time (the Highlights lead-in + assist pass on scoring plays, the game-winner cinema, the pregame ceremony), and the opt-in Full possession-theater mode is the one unbounded spend. Routine plays always blow past, and reduced motion gains nothing.
...
- **Juice the peaks, compress the routine** (the blueprint's watch-pacing rule, grounded here in code): the per-possession plan (`buildPossessionPlan` / `planDurationMs` in `src/components/game/choreography.ts`) owns each possession's budget. In Highlights (the default) routine plays whip by at the 60ms floor while scoring and big plays get a short padded lead-in and the assist pass; in Full mode the linger scales with the play (winner > big > make > routine) and a rebound adds a beat. Reduced motion returns the snappy legacy gap.
- **Motion means something.** In Full mode the floor runs the possession, players advance into a set, cut, pass, attack, rebound, and reset, which is meaningful, choreographed basketball motion (the plan is deterministic per seq), not a jumpy per-possession reshuffle of a static set. Outside that the backdrop holds still and only the focal element moves (the ball, the active player); in Highlights and under reduced motion the set holds and the shooter plus the ball carry the beat.
- **The watch's added duration is budgeted, per mode.** Every spend is named and bounded, or it is a finding:
  - **Highlights (the default) spends a small, bounded lead-in.** Routine plays still blow past at the 60ms floor (they carry the average), but scoring and big plays now hold a short padded lead-in (`HL.pad` ~220ms) plus the assist pass (`HL.pass` ~200ms) so the bucket and the pass that set it up read. This is a real, deliberate, user-requested spend on the ~20 percent of plays that are noteworthy; at the default `brisk` speed a full game's Highlights watch is ~30s. The bound is: only noteworthy plays, only the pad + assist pass + outcome linger, nothing more.
  - **The game-winner cinema** (~250ms, at most once per game, close finishes only) and the **pregame ceremony wipe** (the 3-4 peak games per run) are the other two bounded peaks.
  - **Full mode is opt-in and unbounded by design**: it dramatizes every possession as a real trip up the floor (players run, pass, attack, rebound, reset), so a full game runs several minutes. That is sanctioned only because it is opt-in, always skippable, and speed-adjustable.
  - **Reduced motion gains nothing**: every possession collapses to its snappy outcome gap. Adding milliseconds beyond the four spends above, or anything to the reduced-motion path, is a finding.
...
3. New pacing beats inside the DUR window, every scheduler-held duration and gap through `scaled()` (perceptual constants are the named exception), and zero milliseconds added to the watch outside the four named spends (Highlights lead-in + assist pass on noteworthy plays, game-winner cinema, pregame ceremony, opt-in Full mode) with routine plays still blowing past and reduced motion gaining nothing?
