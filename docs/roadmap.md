...
6. **Sprite art and particles.** Particle bursts, ball flight, rim/net ripple, the pokelike map redesign, team secondary colors, and opponent-themed courts shipped in PR6 (see visual-and-data.md). _Also done:_ the visual excitement pass (watch narrative via `momentum.ts`, on-fire escalation, crunch-time dressing, game-winner cinema, the audible coin/stat economy, run-end and champion ceremonies, staged gacha reveals, the run-heat streak flame, and stake-themed tip-off wipes; see visual-and-data.md section 6), and the reactive-crowd pass (bracket-scaled stands, the peak-game apron crowd, edge pulses, crowd swell SFX, hub since-you-left deltas, and the crest-shelf ceremony; see visual-and-data.md section 7). Still ahead: hand-drawn players and court (beyond the procedural sprites), and multi-frame dunk and block animations.
