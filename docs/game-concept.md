...
1. **Set your five.** Pick a starter for each of the five floor slots (PG, SG, SF, PF, C). Ratings and how players fit together decide games, so chase synergies (speedy backcourts, twin towers, lockdown wings). This is the only routine pre-game decision (a legend chase adds the optional SHOWCASE call, below).
...
- **Favor makes the chase directed**: every un-owned player who logs minutes in a won game banks FAVOR toward owning them, kept even when the run is lost. Favor converts to collection copies, steers the scout machines (including an explicit pinned target), weights recruit re-offers toward past teammates, and picks which legend the once-per-run reveal brings. Getting *your* favorite onto the roster is a chase you can aim, not a lottery. See [favor-system.md](favor-system.md).
- **Top-tier players are EARNED, never farmed**: S-class copies bank only from medium difficulty up (easy banks letters of intent, a favor floor), and every S+ legend signs through a two-mark SIGNATURE CARD: prove their bespoke moment with them on loan, then win a championship together, both on difficulty-floored S ladders. The chase is a deliberate play, not a passive hope: the pregame SHOWCASE call bends the game plan toward the legend's moment at a visible cost, and the run reads back every attempt (qualification, a live tracker, near-miss lines). A moment-gated Legacy Contract is the deterministic buyout, and player growth itself is usage-gated: careers of wins, MVPs, and titles (LEGACY) open the deepest locker ranks and the Icon Perk slot. See [signature-signings.md](signature-signings.md).
