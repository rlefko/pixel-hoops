...
5. Run a **feel** subagent when the change touches anything the player sees, hears, or feels (any change under `src/audio/` or `assets/audio/` counts): check it against the feel conventions in `docs/feel-conventions.md` (the `feel-review` skill runs this check end to end).
